  ...
  try {
    chrome.tabs.sendMessage(tabId, message, (response) => {
      if (chrome.runtime.lastError) {
        console.warn('[Popup] Message send failed:', chrome.runtime.lastError.message);
        // Don't throw, just log - some operations can continue without response
      }
      if (callback) callback(response);
    });
  } catch (error) {
    console.error('[Popup] Tab sendMessage error:', error);
  }
}
  ...
  port.onDisconnect.addListener(() => {
    ...
    setTimeout(() => {
      try {
        ...
        console.log('[Popup] Reconnected successfully');
        ...
        console.error('[Popup] Reconnection failed:', error);
        showStatus('error', '❌ Connection failed. Please close and reopen the extension.');
      }
    }, 500);
  });
}
...
function handleExtractedData(data) {
  ...
  // error as fatal when we actually captured some — surface a warning and still let the
  // user download what was collected.
...
extractBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (reelMatch) {
      ...
      setTimeout(() => {
        if (!listenerCleanedUp) {
          listenerCleanedUp = true;
          chrome.tabs.onUpdated.removeListener(listener);
          console.warn('[Popup] Tab listener cleanup timeout - tab did not complete in 30s');
    ...
    // Progress-aware poll: large posts can take minutes. The background also pushes
    // 'currentData' on completion, so this mainly drives the button state. Stop as soon as
    // a comments array exists (extraction returned — complete or partial) or the cap elapses.
